...
def _(mo):
    _fig = mo.md("![](https://media.licdn.com/dms/image/v2/D4E22AQFZFRSwwzCSqQ/feedshare-shrink_2048_1536/feedshare-shrink_2048_1536/0/1725003016027?e=2147483647&v=beta&t=oBH1s4V8N0wKCOJQakA_wrwgFrixs56S0s_QafZOvbA)")
    _text = mo.md(
        """
...
def _():
    ...
    generator = pipeline("text-generation", # the task
                         model="gpt2", # the model
                         device="mps") # "mps", "cpu", or "cuda" depending on your hardware

    greedy_output = generator("Hi there! ",
                              do_sample = False,
                              max_new_tokens=20,
                              pad_token_id=generator.tokenizer.eos_token_id
                              )
    print(greedy_output[0]["generated_text"])
    return generator, greedy_output, pipeline
...
@app.cell(hide_code=True)
def _(generator):
    beam_output = generator("Hi there! ",
                              do_sample = False,
                              max_new_tokens=20,
                              pad_token_id=generator.tokenizer.eos_token_id,
...
@app.cell
def _(generator):
    top_k_output = generator("Hi there! ",
                              do_sample = True,
                              max_new_tokens=20,
                              pad_token_id=generator.tokenizer.eos_token_id,
...
@app.cell
def _(generator):
    top_p_output = generator("Hi there! ",
                              do_sample = True,
                              max_new_tokens=20,
                              pad_token_id=generator.tokenizer.eos_token_id,
...
@app.cell(hide_code=True)
def _(mo):
    mo.md("""<center><b>Everything together</b></center>
