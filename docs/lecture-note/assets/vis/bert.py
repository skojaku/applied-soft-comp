...
def _(mo):
    mo.md("""
    Each token embedding can be retrieved by:
    """)
    return
...
def _(model, tokenizer, torch, train_data):
    ...
    return all_embeddings, all_labels, all_sentences
...
def _():
    ...
    return (model_masked_lm,)


@app.cell(hide_code=True)
def _(mo):
    mo.md("""
    Now, let's predict the masked word in the sentence.
    """)
    return
