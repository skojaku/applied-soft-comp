...
# PCA
scatter = axes[0].scatter(X_scurve_pca[:, 0], X_scurve_pca[:, 1], c=color,
...
# MDS
scatter = axes[1].scatter(X_scurve_mds[:, 0], X_scurve_mds[:, 1], c=color,
                          cmap='viridis', alpha=0.6, s=20)
axes[1].set_xlabel('MDS1')
axes[1].set_ylabel('MDS2')
...
The S-curve is a 2D surface with a strong bend in 3D space. PCA projects along the directions of maximum variance, which "flattens" the S-shape and loses the smooth color gradient. MDS does better---it partially recovers the curve structure by preserving distances---but it's not perfect because MDS tries to preserve all pairwise distances, including distances that cut across the curve that shouldn't be preserved.
...
```{python}
#| fig-cap: "t-SNE beautifully straightens the S-curve, preserving local neighborhood structure"
#| fig-width: 14
#| fig-height: 6
#| code-fold: true
...
axes[0].set_title('MDS: Partially Recovers Structure')
sns.despine(ax=axes[0])

# t-SNE
axes[1].scatter(X_scurve_tsne[:, 0], X_scurve_tsne[:, 1], c=color,
                cmap='viridis', alpha=0.6, s=20)
axes[1].set_xlabel('t-SNE1')
axes[1].set_ylabel('t-SNE2')
axes[1].set_title('t-SNE: Beautifully Straightens Curve')
sns.despine(ax=axes[1])

plt.tight_layout()
```

t-SNE beautifully straightens the S-curve into a smooth gradient where the color progression is preserved. Points that were neighbors along the curved surface remain neighbors in 2D. Unlike MDS, t-SNE focuses on preserving local neighborhoods rather than all pairwise distances, which allows it to "unfold" the curve more naturally.
