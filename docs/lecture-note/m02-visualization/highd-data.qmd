...
n_samples = 1000
X_swiss, color = make_swiss_roll(n_samples, noise=0.2, random_state=42)

# Apply PCA and MDS
pca_swiss = PCA(n_components=2)
X_swiss_pca = pca_swiss.fit_transform(X_swiss)

mds_swiss = MDS(n_components=2, random_state=41, n_init=1)
...
**Why MDS isn't perfect here:** The Swiss roll has points that are close in Euclidean distance (across the gap in the spiral) but far apart along the manifold surface. MDS tries to preserve both types of distances, creating a compromise that partially unrolls the structure but doesn't fully recover the underlying 2D surface.
