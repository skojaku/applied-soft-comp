...
## Structural Linguistics: The Foundation
...
```{python}
#| fig-cap: "Country-capital relationships preserved in 2D projection. The parallel arrows reveal consistent directional structure—the 'capital of' relationship occupies a stable region of semantic space."
#| fig-width: 12
#| fig-height: 10
...
```{python}
#| fig-cap: "Scientific terms cluster by discipline. Structure isn't imposed—it's discovered from co-occurrence patterns. Biology words share contexts with other biology words, forming a semantic island separate from physics or mathematics."
#| fig-width: 10
#| fig-height: 8
...
Word2vec revealed that meaning is structural, not definitional. We don't understand "dog" by memorizing a description—we understand it through its position in a vast web of relationships. Saussure argued this theoretically in 1916. Firth operationalized it in 1957. Mikolov made it computable in 2013. The algorithm's insight was to make structure explicit through geometry. By compressing co-occurrence patterns into coordinates, it transformed the fuzzy notion of "semantic similarity" into literal distance. Once words have positions, language has topology. And once language has topology, you can navigate meaning through vector arithmetic.
