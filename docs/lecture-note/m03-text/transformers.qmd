...
First, let's see how "bank" sits in embedding space relative to its potential neighbors:
...
Notice "bank" sits neutrally between financial terms (money, loan) on the left and geographical terms (river, shore) on the right. Now try manually adjusting the weights to contextualize "bank":
...
_chart_context = alt.Chart(_df_combined).mark_circle(size=200).encode(
    ...
    tooltip=['word:N', 'x:Q', 'y:Q']
).properties(width=350, height=350, title="Contextualized Bank via Weighted Mixing")
...
q_controls = mo.vstack([mo.md("**Query Transformation (W_Q)**"), q_scale_x, q_scale_y, q_rotate])
k_controls = mo.vstack([mo.md("**Key Transformation (W_K)**"), k_scale_x, k_scale_y, k_rotate])
```
...
_chart_q = alt.Chart(_df_q).mark_circle(size=100).encode(
    ...
    tooltip=['word:N']
).properties(width=200, height=200, title="Query Space")
...
_chart_k = alt.Chart(_df_k).mark_circle(size=100).encode(
    ...
    tooltip=['word:N']
).properties(width=200, height=200, title="Key Space")
