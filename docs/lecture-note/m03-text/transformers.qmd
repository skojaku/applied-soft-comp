...
Processing:
Step 1: Read "The" í Update hidden state hÅ
Step 2: Read "cat" í Update hidden state hÇ (remembers "The")
Step 3: Read "sat" í Update hidden state hÉ (remembers "The cat")
...
Step 6: Read "mat" í Final state hÜ (remembers everything... hopefully)
```
...
- In sentence 1: "bank" attends strongly to "deposited" and "money" í financial institution
- In sentence 2: "bank" attends strongly to "river" and "sat" í river edge
...
attention = np.array([
    [0.6, 0.2, 0.1, 0.05, 0.03, 0.02],  # "The" í mostly attends to "cat"
    [0.1, 0.5, 0.3, 0.05, 0.03, 0.02],  # "cat" í self + "sat"
    [0.05, 0.4, 0.2, 0.15, 0.05, 0.15], # "sat" í "cat" (subject) + "mat" (object)
    [0.05, 0.1, 0.2, 0.3, 0.15, 0.20],  # "on" í preposition, attends to surrounding context
    [0.03, 0.05, 0.05, 0.1, 0.5, 0.27], # "the" í mostly "mat"
    [0.02, 0.05, 0.15, 0.2, 0.15, 0.43] # "mat" í self + "on"
])
...
**Head 1** (Syntactic):
- "sat" í "cat" (subject-verb relationship)
- "on" í "mat" (preposition-object relationship)

**Head 2** (Semantic):
- "cat" í "mat" (both are physical objects)
- "sat" í "on" (action-location relationship)

**Head 3** (Coreference):
- "the" í "cat", "the" í "mat" (determiners to nouns)
...
1. **Tokenization**: Split text into tokens (words or subwords)
   ```
   "Community detection" í ["Community", "detection"]
   ```

2. **Token embeddings**: Convert tokens to vectors (like we saw in the embeddings section)
   ```
   "Community" í [0.23, -0.45, 0.67, ...]
   ```
...
::: {.callout-note}
## Why Stack Layers?
Each layer builds on the previous one, creating hierarchies of abstractionsimilar to how CNNs learn edges í textures í objects in image processing. In text, it's: words í phrases í sentences í concepts.
:::
...
Train BERT on billions of words í learns general language

Fine-tuning (cheap, many times):
Train on 1,000 medical abstracts í learns medical language
Train on 5,000 legal documents í learns legal language
```
...
- More data í better performance
- More parameters í better performance
- Bigger models í emergent abilities (reasoning, math, code)
...
attention = np.array([
    [0.5, 0.3, 0.1, 0.05, 0.05],  # "The" í "scientist"
    [0.2, 0.5, 0.2, 0.05, 0.05],  # "scientist"
    [0.05, 0.3, 0.4, 0.1, 0.15],  # "published" í "scientist", self
    [0.05, 0.6, 0.1, 0.2, 0.05],  # "her" í "scientist" (coreference!)
...
ax.set_xlabel("Attends To", fontsize=12, fontweight='bold')
ax.set_ylabel("Word", fontsize=12, fontweight='bold')
ax.set_title("Attention: Resolving 'her' í 'scientist'", fontsize=14, fontweight='bold')
...
When you use an LLM:
1. Text í Embeddings (tokens to vectors)
2. Embeddings í Transformer layers (attention + feed-forward)
3. Transformer output í Task-specific head (classification, generation, etc.)
...
**Next**: [Word Embeddings: Where It Started í](word-embeddings.qmd)
