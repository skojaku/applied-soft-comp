...
//| echo: false
{
  ...
  function update() {
    ...
    // Add all original vectors
    originalVectors.forEach(item => {
      data.push({
        name: item.name,
        x: item.vector[0],
        y: item.vector[1],
    ...
    // Apply transformations to each vector
    originalVectors.forEach(item => {
      ...
      data.push(
        { name: `q_${item.name}`, x: qVec[0], y: qVec[1], type: "Query" },
        { name: `k_${item.name}`, x: kVec[0], y: kVec[1], type: "Key" },
        { name: `v_${item.name}`, x: vVec[0], y: vVec[1], type: "Value" }
      );
    });
