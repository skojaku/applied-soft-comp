...
```{ojs}
//| echo: false
qScaleXSlider = createQKVSlider(0.1, 2, 0.1, 150, 1.0, "Q Scale X")
```

```{ojs}
//| echo: false
qScaleYSlider = createQKVSlider(0.1, 2, 0.1, 150, 1.0, "Q Scale Y")
```

```{ojs}
//| echo: false
qRotateSlider = createQKVSlider(-180, 180, 5, 150, 0, "Q Rotate (deg)")
```

```{ojs}
//| echo: false
kScaleXSlider = createQKVSlider(0.1, 2, 0.1, 150, 1.0, "K Scale X")
```

```{ojs}
//| echo: false
kScaleYSlider = createQKVSlider(0.1, 2, 0.1, 150, 1.0, "K Scale Y")
```

```{ojs}
//| echo: false
kRotateSlider = createQKVSlider(-180, 180, 5, 150, 0, "K Rotate (deg)")
```

```{ojs}
//| echo: false
vScaleXSlider = createQKVSlider(0.1, 2, 0.1, 150, 1.0, "V Scale X")
```

```{ojs}
//| echo: false
vScaleYSlider = createQKVSlider(0.1, 2, 0.1, 150, 1.0, "V Scale Y")
```

```{ojs}
//| echo: false
vRotateSlider = createQKVSlider(-180, 180, 5, 150, 0, "V Rotate (deg)")
```
...
//| echo: false
{
  ...
  // Function to update visualization
  function update() {
    // Get current slider values from the previous visualization
    const qScaleX = qScaleXSlider.slider.value();
    const qScaleY = qScaleYSlider.slider.value();
    const qRotate = qRotateSlider.slider.value();
    const kScaleX = kScaleXSlider.slider.value();
  ...
  // Add event listeners to the sliders from the previous visualization
  qScaleXSlider.slider.on("onchange", update);
  qScaleYSlider.slider.on("onchange", update);
  qRotateSlider.slider.on("onchange", update);
  kScaleXSlider.slider.on("onchange", update);
