...
//| echo: false
{
  ...
  const qScaleSlider = createQKVSlider(0.1, 2, 0.1, 150, 1.0, "Q Scale");
  const qRotateSlider = createQKVSlider(-180, 180, 5, 150, 0, "Q Rotate (deg)");
  const kScaleSlider = createQKVSlider(0.1, 2, 0.1, 150, 1.0, "K Scale");
  const kRotateSlider = createQKVSlider(-180, 180, 5, 150, 0, "K Rotate (deg)");
  const vScaleSlider = createQKVSlider(0.1, 2, 0.1, 150, 1.0, "V Scale");
  const vRotateSlider = createQKVSlider(-180, 180, 5, 150, 0, "V Rotate (deg)");

  // Original word vector (bank)
  const originalVector = [1.0, 0.5];

  // Create plot container
  const plotContainer = document.createElement("div");

  // Function to transform vector
  function transformVector(vec, scale, rotateDeg) {
    const theta = (rotateDeg * Math.PI) / 180;
    const cos = Math.cos(theta);
    const sin = Math.sin(theta);
    const scaledX = vec[0] * scale;
    const scaledY = vec[1] * scale;
    return [
  ...
  function update() {
    // Get current slider values
    const qScale = qScaleSlider.slider.value();
    const qRotate = qRotateSlider.slider.value();
    const kScale = kScaleSlider.slider.value();
    const kRotate = kRotateSlider.slider.value();
    const vScale = vScaleSlider.slider.value();
    const vRotate = vRotateSlider.slider.value();

    // Apply transformations
    const qVec = transformVector(originalVector, qScale, qRotate);
    const kVec = transformVector(originalVector, kScale, kRotate);
    const vVec = transformVector(originalVector, vScale, vRotate);

    // Prepare data for visualization
    const data = [
      { name: "Original (x)", x: originalVector[0], y: originalVector[1], type: "Original" },
      { name: "Query (q)", x: qVec[0], y: qVec[1], type: "Query" },
      { name: "Key (k)", x: kVec[0], y: kVec[1], type: "Key" },
      { name: "Value (v)", x: vVec[0], y: vVec[1], type: "Value" }
    ];
    ...
    const plot = Plot.plot({
      ...
      color: {
        domain: ["Original", "Query", "Key", "Value"],
        range: ["#666666", "#4682b4", "#2e8b57", "#d2691e"]
      },
      marks: [
        // Origin point
        Plot.dot([{x: 0, y: 0}], {
        ...
        // Custom legend at top center
        Plot.dot([
          {x: -1.8, y: 2.8, color: "#666666"},
          {x: -0.6, y: 2.8, color: "#4682b4"},
          {x: 0.6, y: 2.8, color: "#2e8b57"},
          {x: 1.8, y: 2.8, color: "#d2691e"}
        ], {
          x: "x",
          y: "y",
          fill: "color",
        ...
        Plot.text([
          {x: -1.5, y: 2.8, label: "Original"},
          {x: -0.3, y: 2.8, label: "Query"},
          {x: 0.9, y: 2.8, label: "Key"},
          {x: 2.1, y: 2.8, label: "Value"}
        ], {
          x: "x",
          y: "y",
          text: "label",
  ...
  // Add event listeners to sliders
  qScaleSlider.slider.on("onchange", update);
  qRotateSlider.slider.on("onchange", update);
  kScaleSlider.slider.on("onchange", update);
  kRotateSlider.slider.on("onchange", update);
  vScaleSlider.slider.on("onchange", update);
