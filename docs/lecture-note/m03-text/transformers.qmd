...
weights = [bankWeight, moneyWeight, riverWeight]
total = weights.reduce((a, b) => a + b, 0)
normalizedWeights = total > 0 ? weights.map(w => w / total) : [0, 0, 0]
...
originalData = contextWords.map((word, i) => ({
  word: word,
...
data = [...originalData, ...contextualizedData]
...
```{ojs}
//| echo: false
html`<div style="display: flex; align-items: center; gap: 40px; justify-content: center;">
...
//| echo: false
// Word embeddings in 2D space
attentionWords = ["bank", "money", "loan", "river", "shore"]
attentionEmbeddings = [
  [0.0, 0.0],    // bank (center)
  [-0.8, -0.3],  // money
  [-0.7, -0.6],  // loan
...
Q = transformEmbeddings(attentionEmbeddings, qScaleX, qScaleY, qRotate)
K = transformEmbeddings(attentionEmbeddings, kScaleX, kScaleY, kRotate)

// Compute attention scores (Q @ K^T)
scores = Q.map(q => K.map(k => q[0] * k[0] + q[1] * k[1]))

// Apply softmax to each row
attentionWeights = scores.map(row => {
...
qData = attentionWords.map((word, i) => ({
  word: word,
  x: Q[i][0],
  y: Q[i][1]
}))
...
kData = attentionWords.map((word, i) => ({
  word: word,
  x: K[i][0],
  y: K[i][1]
}))
...
// Combined plots
attentionPlots = html`<div style="display: flex; flex-direction: column; align-items: center; gap: 20px;">
...
```{ojs}
//| echo: false
html`<div style="display: flex; align-items: flex-start; gap: 40px; justify-content: center;">
