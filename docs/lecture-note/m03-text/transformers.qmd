...
viewof bankWeight = Inputs.range([0, 1], {value: 1.0, step: 0.01, label: "Bank Weight"})
viewof moneyWeight = Inputs.range([0, 1], {value: 0, step: 0.01, label: "Money Weight"})
viewof riverWeight = Inputs.range([0, 1], {value: 0, step: 0.01, label: "River Weight"})

{
  ...
  const sliders = html`<div style="display: flex; flex-direction: column; gap: 15px;">
    ${viewof bankWeight}
    ${viewof moneyWeight}
    ${viewof riverWeight}
  </div>`;
...
// Query transformation controls
viewof qScaleX = Inputs.range([-2, 2], {value: 1.0, step: 0.1, label: "Q Scale X"})
viewof qScaleY = Inputs.range([-2, 2], {value: 1.0, step: 0.1, label: "Q Scale Y"})
viewof qRotate = Inputs.range([-180, 180], {value: 0, step: 5, label: "Q Rotate (deg)"})

// Key transformation controls
viewof kScaleX = Inputs.range([-2, 2], {value: 1.0, step: 0.1, label: "K Scale X"})
viewof kScaleY = Inputs.range([-2, 2], {value: 1.0, step: 0.1, label: "K Scale Y"})
viewof kRotate = Inputs.range([-180, 180], {value: 0, step: 5, label: "K Rotate (deg)"})

{
  ...
  const qPlot = Plot.plot({
    ...
    marks: [
      ...
      Plot.text(qData, { x: "x", y: "y", text: "word", dy: -12, fontSize: 10, fontWeight: "bold" }),
      Plot.text([{ x: 0, y: 4.5 }], { x: "x", y: "y", text: () => "Query Space", fontSize: 12, fontWeight: "bold" })
    ]
  });
  ...
  const kPlot = Plot.plot({
    ...
    marks: [
      ...
      Plot.text(kData, { x: "x", y: "y", text: "word", dy: -12, fontSize: 10, fontWeight: "bold" }),
      Plot.text([{ x: 0, y: 4.5 }], { x: "x", y: "y", text: () => "Key Space", fontSize: 12, fontWeight: "bold" })
    ]
  });
  ...
  const qControls = html`<div style="display: flex; flex-direction: column; gap: 10px;">
    <div style="font-weight: bold; margin-bottom: 5px;">Query Transformation (W_Q)</div>
    ${viewof qScaleX}
    ${viewof qScaleY}
    ${viewof qRotate}
  </div>`;

  const kControls = html`<div style="display: flex; flex-direction: column; gap: 10px;">
    <div style="font-weight: bold; margin-bottom: 5px;">Key Transformation (W_K)</div>
    ${viewof kScaleX}
    ${viewof kScaleY}
    ${viewof kRotate}
  </div>`;

  const controls = html`<div style="display: flex; flex-direction: column; gap: 30px;">
    ${qControls}
    ${kControls}
  </div>`;
