...
In S4 and S5, you modified \texttt{analysis.py} to add feature X. In S6, your colleague modified \texttt{README.md} to add documentation.

\begin{enumerate}[resume]
    \item You both want to combine your work. You changed line 5 in \texttt{analysis.py} to \texttt{result = data * 2}. Your colleague also changed line 5 to \texttt{result = data * 3}. What problem occurs when you try to combine the work?

    \vspace{3cm}

    \item Your team keeps snapshots on a backup server. When you want to send your snapshots to the server, what would you call this operation? When you want to retrieve snapshots from the server, what would you call that operation?

    \vspace{3cm}

    \item Before combining work into the main path, someone else on your team reviews your code. Why might this code review step be useful?
