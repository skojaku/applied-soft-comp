...
How can numbers capture the richness of visual information? The answer lies in spatial structure. Unlike a spreadsheet where row order doesn't matter, the arrangement of numbers in an image is everything. Neighboring pixels relate to each other, forming edges, textures, and patterns.
...
Why does this matter for machine learning? Fully connected networks treat every input independently, ignoring spatial relationships. Convolutional networks, which we'll explore soon, are designed specifically to exploit this structure.
