...
Now the model cannot hallucinate. It must return valid JSON that matches the schema. If it tries to return `{"tool_nam": ...}` (typo), the generation fails and retries internally.
...
model = LiteLLMModel(
    model_id="ollama/glm-4.6:cloud",  # LiteLLM requires "ollama/" prefix
...
Let's try a simpler multi-step query:

```{python}
query = "What is the correlation between Weight and Length1?"
result = agent.run(query)
print(result)
```

For this query, the agent must:
1. First call `inspect_data` to discover that Weight and Length1 exist
2. Then use `find_correlation` to calculate the correlation between them
