...
Run the agent and watch it choose the right tool:

```{python}
# Simple single-step task
query = "Generate 30 random numbers from a Gaussian distribution with mean 10 and std 2"
...
The agent executes a ReAct loop:

1. **Thought**: "The user wants random numbers from a Gaussian distribution"
2. **Action**: Chooses `generate_random` tool and calls it with `mean=10, std=2, size=30`
3. **Observation**: Receives output: "Generated 30 samples: mean=10.02, std=1.98"
4. **Final Answer**: Returns the result to the user

This is the core ReAct pattern. For more complex multi-step tasks, the agent chains multiple tool calls together, using the observation from one step to inform the next action.
