...
class QueryDataTool(Tool):
    name = "query_data"
    description = "Query the fish dataset using SQL. The table is called 'df'. Available columns: Species, Weight, Length1, Length2, Length3, Height, Width"
    inputs = {
        "sql_query": {"type": "string", "description": "SQL query to execute (use 'df' as table name)"}
    }
    output_type = "string"
...
Notice the **description** field. This is the agent's only way to understand when to use the tool. The description tells the agent that this tool accepts SQL queries and lists the available columns. Without this context, the agent would never know when to call this tool.
...
# Create the agent with our three tools
...
2. **Action**: Chooses `query_data` tool and constructs SQL: `SELECT Species, AVG(Weight) as avg_weight FROM df GROUP BY Species ORDER BY avg_weight DESC LIMIT 1`
