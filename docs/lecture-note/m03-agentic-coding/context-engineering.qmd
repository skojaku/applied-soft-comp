...
**The Setup**: Google Antigravity connects to external MCP servers through a configuration file. The server exposes tools that the agent can call on demand. We'll wire Context7 into Antigravity so the agent can fetch up-to-date library docs without polluting the context window.
...
**Step 2: Add the Context7 Server**

You have two options: remote HTTP connection (recommended) or local installation.

**Option A: Remote Connection** (No installation required)
...
This spawns the MCP server as a subprocess. The `npx` command fetches the latest version automatically.
...
Without context7, the agent hallucinates method signatures or relies on training data from 2023. With context7, the agent fetches the exact API surface for the version you specify. The MCP protocol isolates the retrieval mechanism from the reasoning loop. The agent doesn't "know" pandas—it knows how to ask for pandas docs when needed. This is **progressive disclosure** in action: load information just-in-time, not just-in-case.

**Critical Constraint: Tool Budget**

Antigravity enforces a tool limit. If you exceed ~50 active tools across all MCP servers, performance degrades. If you hit 80+, Antigravity may refuse to load. When adding context7, audit your existing MCP servers. Disable unused tools. The ideal configuration exposes 20-30 tools total: enough flexibility, not enough to overwhelm the model's function-calling budget.
