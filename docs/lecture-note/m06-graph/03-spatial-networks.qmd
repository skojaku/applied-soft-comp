...
How do we bridge spectral and spatial perspectives? ChebNet [@defferrard2016convolutional] achieves this using Chebyshev polynomials. The key insight is that we can approximate spectral filters without computing full eigendecompositions.
...
Can we simplify further? While ChebNet offers principled spectral approximation, @kipf2017semi proposed an even simpler variant that became one of the most widely used graph neural networks.
...
What about scalability? GCN processes entire graphs at once, limiting its applicability to large graphs. GraphSAGE [@hamilton2017graphsage] introduced an **inductive** framework that generates embeddings by sampling and aggregating features from neighborhoods.
...
Let's ask a fundamental question. What is the maximum discriminative power of graph neural networks? Graph Isomorphism Networks [@xu2018how] provide an answer.
...
@xu2018how prove that GIN can distinguish any graphs that WL can distinguish. This makes it maximally powerful within the class of message-passing neural networks.
