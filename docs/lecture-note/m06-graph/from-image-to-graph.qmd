...
This module bridges the gap between image convolution and graph neural networks.

You'll learn:

- How to **think of image convolution as a graph operation** where pixels are nodes and neighbors are connected.
- The **spectral perspective on graphs** through the graph Laplacian and eigenvalue decomposition.
- How **spectral filters** control frequency components on graphs, just like Fourier transforms on signals.
- Why we need to move **from spectral to spatial methods** for practical graph neural networks.
:::
...
1. **Low-pass Filter**:
   ...
   This filter preserves low frequencies (small λ), suppresses high frequencies (large λ), and results in smoother signals.

2. **High-pass Filter**:
   $$h_{\text{high}}(\lambda) = \frac{\alpha\lambda}{1 + \alpha\lambda}$$
   This filter preserves high frequencies, suppresses low frequencies, and emphasizes differences between neighbors.
...
Building on this idea, @bruna2014spectral added a nonlinearity to the filter and proposed a spectral convolutional neural network (GCN):
