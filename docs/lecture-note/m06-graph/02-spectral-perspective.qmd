...
1. **Computational cost**: Computing eigendecomposition requires $\mathcal{O}(N^3)$ operations, prohibitive for large graphs with millions of nodes.

2. **Lack of spatial locality**: Learned filters operate on all eigenvectors, meaning every node can influence every other node. This loses the inductive bias of locality that makes CNNs powerful.
