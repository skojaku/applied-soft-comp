...
**For this course, you'll use GitHub Classroom.** Your instructor will provide you with an assignment invitation link through Brightspace. When you click that link, GitHub Classroom automatically creates a personal repository just for you.

::: {.column-margin}
**GitHub Classroom:** Our classroom is [Applied Soft Computing Spring 2026](https://classroom.github.com/classrooms/146034511-applied-soft-computing-spring-2026). Assignment links will be distributed through Brightspace.
:::

After accepting the assignment, you'll have a repository URL that looks like:

```
https://github.com/sk-classroom/git-practice-assignment-yourname
```

Now connect your local `git-practice` repository to this GitHub Classroom repository. The `git remote add` command tells your local repository about a remote location. The name `origin` is conventional for your primary remote server.

Navigate to your `git-practice` directory and run (replace `yourname` with your GitHub username):

```bash
git remote add origin https://github.com/sk-classroom/git-practice-assignment-yourname.git
git branch -M main
...
If you get an error about the remote already existing, remove it first:
...
Refresh your GitHub repository page. Your code is now online! You can see your commits, browse your files, and the instructor can review your work.
...
GitHub serves as a backup and collaboration hub. If you're working on multiple computers, you can clone your GitHub Classroom repository anywhere:
...
If you need to pull the latest changes (for example, if you made edits on GitHub's website), use:
...
After completing all the Git exercises above (creating commits, working with branches, resolving merge conflicts, and pushing to GitHub Classroom), submit the following on Brightspace:

1. **GitHub Classroom repository URL** - Submit the link to your repository (e.g., `https://github.com/sk-classroom/git-practice-assignment-yourname`)

2. **Verification checklist** - Your repository should contain:
   - The `analysis.py` file with both `calculate_mean()` and `calculate_median()` functions
   - At least 5 commits showing your progression through the exercises
   - Multiple branches: `add-median`, `use-numpy-mean`, `add-mean-docstring`
   - A merge commit demonstrating conflict resolution
   - All branches pushed to GitHub (verify by clicking "branches" on GitHub)
...
**Tip:** Before submitting, visit your repository on GitHub and verify everything is visible. Check the commit history, branches, and files.
:::
